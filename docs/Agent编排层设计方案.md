...
> **阅读提示**：本文为 2026-09-06 的历史设计/里程碑快照，文中「57 工具 / 24 技能 / 415/443 测试」均指当时的数量；**最终交付态见 `docs/第4组单元测试报告.md`（59 工具 / 26 技能 / 558 测试全绿）**，Mock 亦同数为 59，实现口径以最终报告为准。
...
> （57 工具 / 26 技能 / 415 测试全过 / 真机 QQ·Edge·豆包 / Agent 可调组合技能 / 截图理解 / 邮件全链路 send_email / QQ『先读后回』read_qq_chat / 联网白名单 / HTTP 联网搜索）；彼时缺一层 LLM 驱动的"脑"，由本层补齐
...
13. **QQ『先读后回』= 只读技能 read_qq_chat（2026-09-06，回应"助手不会变通"的真机反馈）**：用户让它"找 QQ 某群、看聊天回一句话"，它答"send 技能只能发、看不到聊天历史"。**根因是能力没暴露，不是模型笨**：ReAct 模型能"想"的范围只有上下文里出现的函数——QQ 当时只暴露了能发的 `app_send_message`（HIGH、发完截图），没有"读"，读能力对它而言**不存在**；它不会自己"截个图看"因为 `screen_inspect`/技能使用同样需要模型先知道。解法＝把"读"也做成白名单函数，让『先看再回』成为模型天然路径：**read_qq_chat**（READ_ONLY、不外发；`_AGENT_SKILL_SCHEMAS` 入白名单、`SKILL_META` READ_ONLY → 任何授权档位不进确认门；Agent 分发时同样默认强制 `verify_ocr=True`）——搜索进入目标会话 → OCR 核对会话标题（`make_ocr_verify`，目标不在结果前几行就失败关闭、绝不读到别的会话）→ 截图裁聊天消息区（布局 `_MSG_AREA` 分数）→ `core.vision_bridge` 视觉桥转写最近聊天返回 transcript/会话名/截图（缺视觉源报错，不假装读到）。同一批把**联网也白名单化**：既有浏览器组合技能 `browser_search`（LOW）/`browser_extract`（READ_ONLY）提升为模型可调函数，白名单 2→5（app_send_message / read_qq_chat / send_email / browser_search / browser_extract，技能 25→26、工具 57 不变）。SYSTEM_PROMPT 新守则：QQ 找群看聊天再回＝先 `read_qq_chat` 再 `app_send_message`；联网查资料＝`browser_search`（bing/baidu）→ `browser_extract`（读正文）并标注来源、不编造。mock 技能库/白名单对等。配套 `tests/test_agent_read_web.py` 11 例 + `test_agent_skills` +2（schema 只读）→ **391 → 404 全绿**（16 个测试文件）。
14. **联网搜索改 HTTP 直连 = 稳定的函数才配给模型（2026-09-06，决策 13 的"能查"落地后暴露的可靠性问题）**：白名单接上后真机让 miniyu 搜 CSGO 赛果 → 反复"搜索超时"约 3 分钟。**根因不是模型，是函数不稳定**：旧 `browser_search` 走无头浏览器"打开引擎页 → type_text → CDP 合成回车"，但**合成回车在必应/百度都不触发表单提交**（实测词已打进输入框、URL 停在首页、body 不变），内部 `wait_for(结果文本)` 每次必 10s 超时；模型重试几次就把偶发延迟放大成"卡死几分钟"。百度另有"安全验证"人机墙，直接 GET 结果页也被拦。修复＝新增 `core/web_search.py`（纯标准库 urllib/gzip/html/re，零新增依赖）：**HTTP 直接请求必应搜索结果页**（UA 伪装桌面 Edge、Accept-Encoding 只声明 gzip/deflate）→ 解析 `<li class="b_algo">` 为 {标题/链接/摘要}——与云端工具（Claude Code 的 WebSearch）同构、无状态可离网单测（喂样本 HTML 验证解析）。两个可靠性细节：①基址**直连 `cn.bing.com`** 而非 `www.bing.com`——后者会 302 跳 cn，CN 网络下该二次 TLS 握手实测偶发被对端重置（WinError 10054）；②**3 次连接级重试**（URLError/超时/ConnectionReset，0.5s 退避；HTTP 4xx/5xx 与解压失败不重试直接报错）。实测单次 **~0.6s** 返回 5 条。engine schema 收敛为仅 `bing`（百度未接入），FAQ/技能表同步。**工程观点：给 LLM 的联网"开关"必须稳到单次秒回**——一个会卡几十秒才失败的函数，等于让模型在幻觉上更耐心。配套 `tests/test_web_http.py` 11 例（离网：b_algo 解析清洗 / 相对链接过滤 / top 截断 / 摘要截断 / 空结果 / baidu 拒接 / 异常包装 / 瞬断重试后成功）→ **404 → 415 全绿**（17 个测试文件）。
