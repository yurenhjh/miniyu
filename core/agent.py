...
SYSTEM_PROMPT = (
    ...
    "元素都带一个短 handle（形如 e3），它是当前页面状态下的**短生命周期引用**。点击/输入/读取一律优先用这个 handle："
    "browser_click(target='e3') / browser_type(target='e3', text=...) / browser_read_text(target='e3')。只能使用最近一次工具返回的 handle，**不得"
    ...
    "同样，**不要根据自然语言目标自行猜测或构造 CSS selector**——find 得到 handle 后，后续 click/type/read 一律"
    "优先复用该 handle；确需按选择器定位时，才显式用 browser_read_text(selector=...) 这类 selector 参数。"
